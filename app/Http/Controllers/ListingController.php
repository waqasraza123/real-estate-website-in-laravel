    ...
    public function filterListings(Request $request){
        ...
        if($listings){
            foreach ($listings as $listing){
                if($listing->lat != '') {
                    if($listing->listing_type == '2'){
                        $new = '[' . $listing->lat . ' , ' . $listing->lng . ', "images/pin-apartment.png" , "/assets/images/'.$this->listingImage->where('listing_id' , $listing->id)->whereNotNull('featured')->pluck('image')->first().'" , "'.$listing->address.'" , "'.$listing->rent.'" , "'.route('singleListing' , ['id' => $listing->id , 'title' => $listing->title]).'"],';
                    }elseif($listing->listing_type == '7'){
                        $new = '[' . $listing->lat . ' , ' . $listing->lng . ', "images/pin-house.png" , "/assets/images/'.$this->listingImage->where('listing_id' , $listing->id)->whereNotNull('featured')->pluck('image')->first().'" , "'.$listing->address.'" , "'.$listing->rent.'" , "'.route('singleListing' , ['id' => $listing->id , 'title' => $listing->title]).'"],';
                    }elseif($listing->listing_type == '5'){
                        $new = '[' . $listing->lat . ' , ' . $listing->lng . ', "images/pin-commercial.png" , "/assets/images/'.$this->listingImage->where('listing_id' , $listing->id)->whereNotNull('featured')->pluck('image')->first().'" , "'.$listing->address.'" , "'.$listing->rent.'" , "'.route('singleListing' , ['id' => $listing->id , 'title' => $listing->title]).'"],';
                    }elseif($listing->listing_type == '3'){
                        $new = '[' . $listing->lat . ' , ' . $listing->lng . ', "images/pin-land.png" , "/assets/images/'.$this->listingImage->where('listing_id' , $listing->id)->whereNotNull('featured')->pluck('image')->first().'" , "'.$listing->address.'" , "'.$listing->rent.'" , "'.route('singleListing' , ['id' => $listing->id , 'title' => $listing->title]).'"],';
                    }else{
                        $new = '[' . $listing->lat . ' , ' . $listing->lng . ', "images/pin-land.png" , "/assets/images/'.$this->listingImage->where('listing_id' , $listing->id)->whereNotNull('featured')->pluck('image')->first().'" , "'.$listing->address.'" , "'.$listing->rent.'" , "'.route('singleListing' , ['id' => $listing->id , 'title' => $listing->title]).'"],';
                    }
                    array_push($langLtd, $new);
